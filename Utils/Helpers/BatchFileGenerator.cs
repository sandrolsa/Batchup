...
namespace Batchup.Utils.Helpers
{
    ...
    {
        public void CriarArquivoBat(ConfigConexaoModel conexao, ConfigBackupModel config, string destino)
        {
            string conteudo = GerarConteudoBat(conexao, config);
            File.WriteAllText(destino, conteudo);
        }

        public string GerarConteudoBat(ConfigConexaoModel conexao, ConfigBackupModel config)
        {
            string pastaBat = Path.Combine(config.LocalBackup, "BAT");
            return $@"
@ECHO OFF
